...
The click is inserted before Media3's speed processor, so song and click are time-stretched together. Media3 passes `StreamMetadata.positionOffsetUs` when it flushes the processor, allowing load, seek, discontinuity, and loop restart to reset the click cursor from the pipeline's real media origin. Generation IDs prevent stale React snapshots from an earlier operation.
